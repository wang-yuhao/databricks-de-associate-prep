...
## Task 7 — Generating Bundles and Variable Override Precedence

📖 **Context**: The exam tests two related skills: converting an existing, manually-created job into a bundle definition via `bundle generate`, and knowing the precedence order when the same variable is set in multiple places.

🛠️ **Instructions**:

### Step 1 — Generate a bundle resource from an existing job:

```bash
# Reverse-engineer an existing job into YAML you can commit and version
databricks bundle generate job --job-id 12345 --config-dir resources/

# This writes resources/<job_name>.job.yml and downloads any referenced
# notebooks into a local src/ folder so the bundle is self-contained
```

### Step 2 — Set the same variable three different ways:

```yaml
# databricks.yml
variables:
  catalog:
    default: training_uc          # (3) lowest precedence: top-level default

targets:
  dev:
    variables:
      catalog: training_uc_dev    # (2) middle precedence: target-level override
```

```bash
# (1) highest precedence: explicit CLI flag
databricks bundle deploy --target dev --var="catalog=training_uc_scratch"

# equivalent via environment variable
export BUNDLE_VAR_catalog=training_uc_scratch
databricks bundle deploy --target dev
```

### Step 3 — Confirm which value actually deployed:
...
✅ **Expected outcome**:
- `bundle generate` produces YAML plus any notebook source files needed to redeploy the job as code
- You can order the three variable sources from highest to lowest precedence: CLI `--var` / `BUNDLE_VAR_*` env var → target-level `variables:` → top-level `variables: default:`

⚠️ **Exam trap**: Assuming a target-level `variables:` override always wins. Wrong! An explicit `--var` flag (or its `BUNDLE_VAR_<name>` environment variable equivalent) passed at deploy time **overrides everything**, including target-level values.
...
11. What does `databricks bundle generate job --job-id <id>` produce?
12. Between a CLI `--var` flag and a target-level `variables:` entry, which one wins?
...
- **`bundle generate`**: reverse-engineers an existing job/pipeline into bundle YAML + source files
- **Variable precedence** (highest to lowest): CLI `--var` / `BUNDLE_VAR_*` env var → target-level `variables:` → top-level `variables: default:`
